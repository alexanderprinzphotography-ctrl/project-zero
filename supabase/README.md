...
| `20260709120000_ms11a_invoicing_integration.sql` | `company_integrations` (API-Key pro Firma/Provider, verschlüsselt) — kein INSERT/UPDATE-Grant für `authenticated` (additive-Grants-Lektion aus MS 9a proaktiv angewandt: `api_key_encrypted` in keinem Client-Grant); `upsert_company_integration()`/`get_company_integration_secret()`/`set_company_integration_status()` als einzige SECURITY-DEFINER-Schreib-/Lesepfade (nur admin, eigene Firma); `contacts.sevdesk_contact_id` (Kontakt-Verknüpfung, reitet auf bestehender Contacts-Policy) |
| `20260709190000_ms11b_invoices.sql` | `invoices` (reiner Referenz-/Statusspiegel zu sevdesk-Rechnungen aus angenommenen Angeboten), `validate_invoice_company()`-Trigger, RLS wie `quotes` (admin/PL, SELECT ohne Trial-Sperre), **keine DELETE-Policy** (schützt die UNIQUE-Constraint auf `quote_id` als zentrale Idempotenz-Absicherung), additive-Grants-Sperre auf UPDATE (nur `status`/`last_synced_at`/`due_date` clientseitig änderbar — Betrag/Rechnungsnummer sind nach dem Anlegen unveränderlich) |
| `20260709200000_ms11b_fix_invoices_update_policy.sql` | Korrektur: die UPDATE-RLS-Policy auf `invoices` fehlte — Spalten-Grant und RLS-Policy sind getrennte Schichten, ohne eigene Policy blieb jedes Update wirkungslos (0 Zeilen betroffen, kein Fehler); ergänzt `invoices_update` (admin/PL, ohne `company_is_writable()` für den Status-Sync) |
| `20260710120000_ms12a_quote_share.sql` | `quote_share_links`/`quote_responses` (öffentliches, Token-basiertes Kundenportal `/angebot/<token>`) — `get_quote_share()`/`get_quote_share_items()`/`respond_to_quote_share()` als einzige `SECURITY DEFINER`-Zugriffspfade für die Rolle `anon` (kein generischer RLS-Zugriff für Unauthentifizierte); `quote_responses` für `authenticated` komplett schreibgeschützt (append-only Vertragsnachweis, analog Tagebuch); additive-Grants-Sperre auf `quote_share_links` (nur `revoked_at` clientseitig änderbar) |
| `20260711120000_ms12b_email.sql` | `companies.reply_to_email`/`contact_phone` (inkl. Grant + Nur-Lese-Sperre über `protect_company_settings_when_readonly()`), `email_log` (Versand-Protokoll für Angebots-Mails, append-only — kein UPDATE/DELETE; INSERT normale RLS-Policy statt `SECURITY DEFINER`, da der Schreibende ein bereits authentifizierter interner Akteur ist, anders als beim Kundenportal) |
